...
public enum Dice
{
	...
	Dice(String typeName, TextFormatting typeColor, boolean dyeable, ITag<Item> craftingItem)
	{
		...
		tag = ItemTags.createOptional(new ResourceLocation(DiceMod.ID, DStrings.TAG_DICE + '/' + typeName));
				...
				.withStyle(style ->
						style.withHoverEvent(
								new HoverEvent(
										...
										new TranslationTextComponent(DiceMod.DICE_ROLL_USING_KEY, die.getHoverName()).withStyle(hoverStyle -> getTextComponentStyleColor(die, hoverStyle))
								)
						)
				);
		// formatter:on
	...
	{
		return DiceMod.registrate().object(dice.typeName + '_' + itemName);
	}
	...
	private static ItemEntry<DiceItem> register(Dice dice, String itemName, int sides, boolean dyeable, ITag.INamedTag<Item> sidedTag, NonNullUnaryOperator<ShapedRecipeBuilder> recipePattern)
	{
		...
		return (dyeable ? dyeable(dice, itemName) : generic(dice, itemName))
					...
					*/
					...
					.model((ctx, provider) -> provider.generated(ctx, new ResourceLocation(DiceMod.ID, ITEM_FOLDER + '/' + dice.typeName + '/' + itemName)))
					.recipe((ctx, provider) -> recipePattern
							...
							.save(provider, new ResourceLocation(DiceMod.ID, dice.typeName + '/' + itemName))
					)
				.register();
		// formatter:on
	}
